...
import { isWebPushConfigured } from "@/lib/city-run/push-server";
...
create index if not exists push_subscriptions_rider_id_idx on public.push_subscriptions (rider_id);
alter table public.push_subscriptions enable row level security;
create policy "push_subscriptions_api_all" on public.push_subscriptions for all using (true) with check (true);`,
    });
  } catch (error) {
